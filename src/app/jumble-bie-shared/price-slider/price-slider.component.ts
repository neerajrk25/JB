declare var noUiSlider, wNumb;
import { Component, ElementRef, EventEmitter, forwardRef, Input, OnInit, OnChanges, Output, Renderer2 } from '@angular/core';
...
export class PriceSliderComponent implements ControlValueAccessor, OnInit, OnChanges {

    public slider: any;
    public handles: any[];
    @Input() public disabled: boolean; // tslint:disable-line
    @Input() public behaviour: string = 'tap-drag';
    @Input() public connect: boolean = true;
    @Input() public limit: number;
    @Input() public min: number = 1000;
    @Input() public max: number = 20000;
    @Input() public step: number = 100;
    @Input() public format: any = wNumb({
        decimals: 0,
        thousand: ',',
        prefix: 'Rs.',
    });
    @Input() public pageSteps: number;
    @Input() public config: any = { tooltips: true };
    @Input() public ngModel: number | number[] = [1000, 5000];
